...
========================================================= */
async function syncEventBudget(
  eventId: Types.ObjectId
) {
  if (!eventId) return;

  const suppliers = (await mongoose
    .model<EventSupplierDoc>(
      "EventSupplier"
    )
    .find({ eventId })
    .select("price")
  ...
  if (
    ...
    console.log(
      "🟡 EventSupplier sync (calculated commitments only):",
      {
