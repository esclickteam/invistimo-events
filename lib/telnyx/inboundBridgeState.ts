...
export async function markBridgeDialing(params: {
  ...
  /*
    Regular $set updates only (no aggregation pipeline).
    Race-safe: answered may arrive before dialing marker; never wipe terminal
    statuses or force status backward.
  */
  let updated = await InboundSoftphoneBridgeAttempt.findOneAndUpdate(
  ...
  // Already progressed past claimed — set dialStartedAt only when still null.
  updated = await InboundSoftphoneBridgeAttempt.findOneAndUpdate(
    {
      ...idFilter,
...
export async function markBridgeOutboundCreated(params: {
  ...
  if (outboundLegId) {
    outboundFields.outboundCallLegId = outboundLegId;
  }

  /*
    Regular $set only — no aggregation pipeline.
    claimed/dialing → ringing; answered/bridged/completed keep their status.
  */
  let updated = await InboundSoftphoneBridgeAttempt.findOneAndUpdate(
    {
      ...outboundUnsetFilter,
