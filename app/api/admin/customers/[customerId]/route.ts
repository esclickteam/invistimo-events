...
export async function GET(
  ...
  try {
    ...
    const customer = await CustomerFile.findById(customerId).lean();
    ...
    const [quotes, agreements] = await Promise.all([
      CustomerQuote.find({ customerFileId: customerId }).sort({ createdAt: -1 }).lean(),
    ...
    return NextResponse.json(
      { success: false, error: "שגיאה בטעינת תיק לקוח" },
      { status: 500 }
    );
  }
}