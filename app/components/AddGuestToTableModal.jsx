...
export default function AddGuestToTableModal({ table, guests, onClose }) {
  ...
  // מאזין תמיד לשולחן המעודכן מה-store
  ...
  const seatsArray = useMemo(() => {
    const arr = Array.from({ length: tableData?.seats || 0 }, (_, i) => ({
      index: i,
      guest: null,
    }));
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="relative bg-white rounded-2xl shadow-2xl w-[640px] p-7 max-h-[90vh] overflow-y-auto border border-gray-100">
        <button
          onClick={onClose}
        ...
        {error && (
          <div className="text-red-600 bg-red-50 border border-red-200 text-center py-2 rounded-xl mb-3 font-medium">
            {error}
          </div>
        )}
