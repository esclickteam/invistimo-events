...
export default function ProducerDashboard() {
  ...
  const [openAssignForClientId, setOpenAssignForClientId] = useState<string | null>(null);
  const [savingClientId, setSavingClientId] = useState<string | null>(null);

  // חיפוש עובדים בתוך כל דרופדאון לפי clientId
  const [staffSearchByClientId, setStaffSearchByClientId] = useState<Record<string, string>>({});
  ...
  /* =========================
     Derived flags
  ========================= */
  const isProducer = !!user && user.role === "producer";
  ...
  useEffect(() => {
    ...
    const syncAuthFromServer = async () => {
      ...
      // fallback: ננסה למשוך את המשתמש מה-cookie
      try {
  ...
  }, [authLoading, authResolved, user, isProducer, router]);

  /* =========================
     Redirect if impersonated
  ========================= */
  useEffect(() => {
    if (user?.impersonated) {
      router.replace("/events/production");
    }
  }, [user, router]);
  ...
  }, [authResolved, isProducer, fetchClients, fetchStaff]);
  ...
  /* =========================
     Assignment helpers
  ========================= */
  const isClientAssignedToStaff = useCallback((clientId: string, staff: Staff) => {
    const ids = Array.isArray(staff?.assignedClientIds) ? staff.assignedClientIds : [];
  ...
  // סינון עובדים לפי חיפוש בדרופדאון (שם/אימייל)
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalGuests: withEvent.reduce((sum, c) => sum + (c.event?.totalGuests || 0), 0),
  ...
  return (
    <>
      <ProducerDashboardHeader homeHref="/producer/dashboard" />

      <main className="pt-16 p-6 space-y-10">
        {/* Header */}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {clients.map((client) => {
                  ...
                  const filteredStaff = getFilteredStaffForClient(String(client._id));

                  const location =
                    typeof client.event?.location === "string"
                      ? client.event.location
                      : client.event?.location?.address;
                  ...
                  return (
                    <tr key={client._id} className="border-b hover:bg-slate-50">
                      ...
                      <td className="p-4 font-medium">
                        {client.event ? (
                          `${client.event.approvedCount || 0} / ${client.event.totalGuests || 0}`
                        ) : (
                          <span className="text-slate-400">—</span>
                        )}
                      </td>
                      ...
                      <td className="p-4 relative">
                        ...
                        >
                          ...
                          >
                            {isSavingThisRow
                              ? "שומר..."
                              : assignedStaffNamesForClient(String(client._id))}
                          ...
                          {isOpen && (
                            <div className="absolute z-50 mt-2 min-w-[320px] rounded-xl border bg-white shadow-lg p-2">
                              ...
                              <div className="px-2 pb-2">
                                <input
                                  type="text"
                                  value={staffSearchByClientId[String(client._id)] || ""}
                                  onChange={(e) =>
                              ...
                              <div className="pt-2 mt-1 border-t flex items-center justify-between">
                                <span className="text-xs text-slate-500">
                                  ...
                                  {
                                    staffList.filter((s) =>
                                      isClientAssignedToStaff(String(client._id), s)
                                    ).length
                                  }
                                </span>
